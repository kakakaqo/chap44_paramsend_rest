...
public class RestControllerDemo {
	...
	/*
	 * @RestController의 모든 메소드는 @ResponseBody가 자동으로 붙음(보이지 않지만)
	 * @ResponseBody : 요청한 페이지의 바디에 출력
	 */
	// @ResponesBody // 생략해도 자동으로 붙기 때문에 주석처리
	@RequestMapping(value="/restAction1.do/{name}/{grade}", method= {RequestMethod.GET, RequestMethod.POST})
	public MemberVo action4(@PathVariable("name") String name,
							@PathVariable("grade") int grade, Model model) {
	...
	/*
	 * @RestControll의 모든 메소드는 @ResponseBody가 자동으로 붙음
	 * - @ResponseBody : 요청한 페이지의 바디에 출력
	 * - @RequestBody MemberVo : JSP ajax형태로 요청한 값들을 커맨드 객체 자동 바인딩
	 */
